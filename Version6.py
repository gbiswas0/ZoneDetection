...
wordup = 'image2018-07-04_19-40-09.jpg'
...
for d in range(1, len(discs)+1):
	...
	for h in range(15, 100, 2):
		...
		blank_image = np.zeros((height,width,3), np.uint8)
		cv2.circle(blank_image, (int(newdiscs[d][0]), int(newdiscs[d][1])), h+3, (255, 255, 255), 1)
		...
		#cv2.circle(blank_image, (int(newdiscs[d][0]), int(newdiscs[d][1])), 10, (0, 0, 255), 1 )
		#cv2.imshow("Image", blank_image)
		#cv2.waitKey(0)
		
		p = 3
		innermed = np.median(innervalues)
		innerstd = np.std(innervalues)
		outermed = np.median(outervalues)
		outerstd = np.std(outervalues)
		innermean = np.mean(innervalues)
		outermean = np.mean(outervalues)

		#print(innerq1, innerq3, outerq1, outerq3)
		#print(innervalues, outervalues)

		innervalues = innervalues[abs(innervalues - innermean) < (p * innerstd)]
		outervalues = outervalues[abs(outervalues - outermean) < (p * outerstd)]

		toppercentile = 90
		botpercentile = 10
		innerq1 = np.percentile(innervalues, botpercentile)
		innerq3 = np.percentile(outervalues, toppercentile)
		outerq1 = np.percentile(outervalues, botpercentile)
		outerq3 = np.percentile(outervalues, toppercentile)
		innervalues = innervalues[abs(innervalues) < innerq3]
		outervalues = outervalues[abs(outervalues) < outerq3]
		innervalues = innervalues[abs(innervalues) > innerq1]
		outervalues = outervalues[abs(outervalues) > outerq1]
...
for k in range(1, len(discs)+1):
	...
	if(graddists[k-1] != 10):
		...
		cv2.imshow("Image", blank_image)
		cv2.waitKey(0)
		if(np.mean(zonesats) < 40 and np.median(zonesats) < 40):
