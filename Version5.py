...
grayimg = cv2.cvtColor(newblur, cv2.COLOR_BGR2GRAY)
height, width, channels = imgdif.shape
blank_image = np.zeros((height,width,3), np.uint8)
smalldish = copy.deepcopy(blank_image)

cv2.circle(smalldish, (int(cdX*0.1), int(cdY*0.1)), int(dishrad*downfactor), (255, 255, 255), 5 )
	...
	for h in range(15, 80, 2):
		...
		for i in range(0, len(innercircle)-1):
			...
			while overlap == False:
				...
				if(e == len(disharr)):
					break
				if(np.array_equal(innercircle[i], disharr[e]) == True):
					overlap = True
			print('i', i)
		...
		for j in range(0, len(outercircle)-1):
			...
			while  overlap == False:
				...
				if(f == len(disharr)):
					break
				if(np.array_equal(outercircle[j], disharr[f]) == True):
					overlap = True
		...
		cv2.circle(blank_image, (int(discs[d][0]*downfactor), int(discs[d][1]*downfactor)), 5, (255, 255, 0), 1 )
