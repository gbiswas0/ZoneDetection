...
wordup = 'image2018-07-02_065140-overgrown.jpg'
...
newblur = cv2.GaussianBlur(imgdif, (7,7), 0)
...
cv2.circle(smalldish, (int(cdX*0.1), int(cdY*0.1)), int(dishrad*downfactor), (255, 255, 255), 1 )
cropimg = imgdif[0:int(cdY*0.1)+int(dishrad*downfactor), int(cdX*0.1)-int(dishrad*downfactor):int(cdX*0.1)+int(dishrad*downfactor)].copy()
...
for a in range(1, len(discs)+1):
	...
	if(np.amax(tvals) > math.fabs(np.amin(tvals))):
		if(pvals[np.argmax(tvals)] < 0.005 and np.amax(tvals) > 3.5):
			graddists.append(((np.argmax(tvals)*2)+15))
		else:
			graddists.append(10)
		print(a, np.amax(tvals), np.amin(pvals))
	else:
		if(pvals[np.argmin(tvals)] < 0.005 and np.amin(tvals) < -3.5):
